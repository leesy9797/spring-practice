...
public class BoardRepository {
    ...
    public List<Board> getList(String search, String keyword) {
        ...
        if (!search.equals("") && !search.equals("all")) {
            where = " where b_" + search + " like '%" + keyword + "%'";
        } else if (!search.equals("") && search.equals("all")) {
            where = " where b_title like '%" + keyword + "%' or b_content like '%" + keyword + "%'";
        }
        System.out.println("getList : " + where);
    ...
    public int doUpdate(BoardForm form) {
        ...
        int result = jdbcTemplate.update(
                "update board set b_title = ?, b_content = ?, b_date = ? where b_idx = ? and m_id = ?"
                , form.getB_title(), form.getB_content(), now, form.getB_idx(), form.getM_id()
        );
    ...
    public int regReply(Reply reply) {
        ...
        jdbcTemplate.update("update board set b_replycount = b_replycount + 1 where b_idx = ?", reply.getB_idx());

        return idx;
    }

    public int delReply(int r_idx, int b_idx) {
        jdbcTemplate.update("update board set b_replycount = b_replycount - 1 where b_idx = ?", b_idx);
        jdbcTemplate.update("delete from reply where r_idx = ?", r_idx);
        return r_idx;
    }
    ...
    public int likeOrUnlike(String x, int idx, String mid) {
        ...
        if(x.equals("like")) {
            ...
            parameters.put("m_id", mid);
            parameters.put("g_date", now);

            jdbcInsert.execute(new MapSqlParameterSource(parameters));
            result = 1;
            result += jdbcTemplate.update("update board set b_good = b_good + 1 where b_idx = ?", idx);
        } else {
            result = jdbcTemplate.update("delete from good where b_idx = ? and m_id = ?", idx, mid);
            result += jdbcTemplate.update("update board set b_good = b_good - 1 where b_idx = ?", idx);
        }
